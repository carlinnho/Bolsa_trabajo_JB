...
const MOCK_JOBS = [
  {
    ...
    isNew: true,
    description: "Buscamos un estudiante entusiasta para unirse a nuestro equipo de Atracción de Talento. Aprenderás procesos de reclutamiento de punta a punta, publicación en bolsas de empleo, filtrado curricular y entrevistas iniciales estructuradas.",
    requirements: [
  ...
  },
  {
    ...
    category: "Administración",
    isNew: true,
    description: "Excelente oportunidad para estudiantes de derecho que deseen especializarse en el ámbito laboral y de asesoría corporativa. Formarás parte de un equipo con amplia trayectoria resolviendo desafíos legales para clientes del sector industrial.",
    requirements: [
  ...
  },
  {
    ...
    category: "Ventas",
    isNew: true,
    description: "Buscamos profesionales dinámicos y motivados por el logro comercial para la prospección de clientes corporativos (B2B). Tu función principal será expandir la cartera de empresas contratantes para nuestros servicios de consultoría.",
    requirements: [
  ...
  },
  {
    ...
    category: "Logística",
    isNew: false,
    description: "Para nuestro cliente del sector de seguridad industrial I.seg, nos encontramos en la búsqueda de Operarios de Almacén para su sede en Lima. Persona encargada del orden, despacho y recepción de materiales.",
    requirements: [
      "Secundaria completa.",
  ...
  },
  {
    ...
    category: "Tecnología",
    isNew: true,
    description: "Estamos buscando un Desarrollador Frontend Jr. en React.js para sumarse al equipo de desarrollo tecnológico de nuestro cliente I.seg. Participarás en la optimización de los portales web públicos y de conversión.",
    requirements: [
...
const COMPANAS_CONFIANZA = [
  {
    ...
    render: () => (
      <div className="bg-white rounded-xl p-4 flex flex-col items-center justify-center border border-slate-200/80 shadow-sm w-44 h-28 hover:scale-105 transition-transform duration-200 cursor-pointer">
        <svg className="w-8 h-8 text-slate-800" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
          <circle cx="12" cy="12" r="3" />
        </svg>
        <span className="font-black text-[10px] tracking-widest text-slate-800 mt-3.5 font-heading uppercase">SILVER EYES</span>
      </div>
    )
  },
  {
    ...
    render: () => (
      <div className="bg-white rounded-xl p-4 flex flex-col items-center justify-center border border-slate-200/80 shadow-sm w-44 h-28 hover:scale-105 transition-transform duration-200 cursor-pointer">
        <div className="flex items-center gap-0.5">
          <span className="font-black text-[#14A38B] text-lg sm:text-xl font-heading tracking-tight">true</span>
          <span className="font-bold text-[#A3A3A3] text-lg sm:text-xl font-heading tracking-tight">dent</span>
        </div>
        <svg className="w-5 h-5 text-[#14A38B] mt-2" fill="none" stroke="currentColor" strokeWidth="2" strokeDasharray="3,3" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    render: () => (
      <div className="bg-[#0A0D14] rounded-xl p-4 flex flex-col items-center justify-center shadow-sm w-44 h-28 hover:scale-105 transition-transform duration-200 cursor-pointer text-white">
        ...
        <span className="text-[6px] text-[#A3A3A3] tracking-[0.25em] uppercase mt-1">Producciones</span>
      </div>
    )
  },
  {
    ...
    render: () => (
      <div className="bg-[#096ACC] rounded-xl p-4 flex items-center justify-center shadow-sm w-44 h-28 hover:scale-105 transition-transform duration-200 cursor-pointer text-white">
        <span className="font-black text-2xl sm:text-3xl font-heading tracking-wider uppercase">W|E</span>
      </div>
    )
  },
  {
    ...
    render: () => (
      <div className="bg-white rounded-xl p-4 flex flex-col items-center justify-center border border-slate-200/80 shadow-sm w-44 h-28 hover:scale-105 transition-transform duration-200 cursor-pointer">
        <div className="flex items-center gap-1.5">
          <svg className="w-5.5 h-5.5 text-[#096ACC]/60 fill-current" viewBox="0 0 24 24">
            <path d="M12 2 L22 8 L22 16 L12 22 L2 16 L2 8 Z" />
          </svg>
          <div className="flex flex-col items-start leading-none">
            <span className="font-black text-[#096ACC] text-sm font-heading">mente</span>
            <span className="font-semibold text-[#096ACC] text-sm font-heading">sabia</span>
          </div>
        </div>
      </div>
    )
    ...
    render: () => (
      <div className="bg-white rounded-xl p-4 flex flex-col items-center justify-center border border-slate-200/80 shadow-sm w-44 h-28 hover:scale-105 transition-transform duration-200 cursor-pointer">
        <div className="flex items-center gap-2">
          <svg className="w-6 h-6 text-[#096ACC]" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3" />
          </svg>
          <div className="flex flex-col leading-none">
            <span className="font-black text-[#FDB907] text-xs font-heading">Ares</span>
            <span className="font-black text-[#096ACC] text-xs font-heading">Salud</span>
          </div>
        </div>
      </div>
    )
    ...
    render: () => (
      <div className="bg-white rounded-xl p-4 flex flex-col items-center justify-center border border-slate-200/80 shadow-sm w-44 h-28 hover:scale-105 transition-transform duration-200 cursor-pointer">
        <div className="flex flex-col items-center leading-none">
          <span className="font-black text-sm sm:text-base text-slate-800 tracking-wide font-heading">QUIRO</span>
          <span className="font-bold text-[9px] text-[#096ACC] tracking-widest mt-1">VIDA</span>
        </div>
      </div>
    )
  }
...
export default function Home() {
  ...
  const filteredJobs = useMemo(() => {
    return MOCK_JOBS.filter((job) => {
      const matchSearch =
        ...
        job.company.toLowerCase().includes(appliedFilters.search.toLowerCase()) ||
        job.category.toLowerCase().includes(appliedFilters.search.toLowerCase());

      const matchLocation =
        job.location.toLowerCase().includes(appliedFilters.location.toLowerCase()) ||
        job.locationType.toLowerCase().includes(appliedFilters.location.toLowerCase());
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white font-sans antialiased text-slate-800 relative">
      ...
      {/* 1. HEADER / NAVBAR */}
      <header className="sticky top-0 z-45 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">

          <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate("/")}>
            <div className="w-11 h-11 bg-[#123498] rounded-full flex items-center justify-center shadow-sm">
              <span className="text-white font-extrabold text-lg tracking-tighter">JB</span>
            </div>
            <div className="flex flex-col">
              <span className="font-black text-[#123498] text-base sm:text-lg leading-none tracking-wide font-heading">
                CONSULTORA JB
              </span>
              <span className="font-extrabold text-[#FDB907] text-[10px] sm:text-xs tracking-[0.2em] leading-none uppercase mt-0.5">
                Bolsa de Trabajo
              </span>
            </div>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-[#123498] text-white font-black text-xs uppercase tracking-wider rounded-full hover:bg-[#096ACC] transition-all"
            >
            ...
            <a
              href="#empleos-recientes"
              className="flex items-center gap-2 px-5 py-2.5 text-[#A3A3A3] hover:text-[#123498] font-bold text-xs uppercase tracking-wider transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              onClick={() => alert("Módulo de empresas próximamente disponible")}
              className="flex items-center gap-2 px-5 py-2.5 text-[#A3A3A3] hover:text-[#123498] font-bold text-xs uppercase tracking-wider transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="hidden md:block">
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-[#F46F0B] hover:bg-orange-600 text-white font-black text-xs uppercase tracking-wider rounded-full shadow-sm hover:shadow-md transition-all cursor-pointer"
            >
              <span>Entrar</span>
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="md:hidden flex items-center">
            <button
              ...
              className="p-2 text-[#123498] hover:bg-slate-50 rounded-lg focus:outline-none"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-100 bg-white px-4 py-4 space-y-2.5">
            <button
              onClick={() => {
                setActiveCategory("Todos");
                setAppliedFilters({ search: "", location: "" });
              ...
              className="w-full flex items-center gap-3 px-4 py-3 bg-[#123498] text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-sm"
            >
            ...
            <a
              ...
              className="w-full flex items-center gap-3 px-4 py-3 text-slate-600 hover:text-[#123498] font-black text-xs uppercase tracking-wider rounded-xl hover:bg-slate-50 transition-all"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
            ...
            <button
              onClick={() => {
                alert("Módulo de empresas próximamente disponible");
                setMobileMenuOpen(false);
              }}
              className="w-full flex items-center gap-3 px-4 py-3 text-slate-600 hover:text-[#123498] font-black text-xs uppercase tracking-wider rounded-xl hover:bg-slate-50 transition-all text-left"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#F46F0B] hover:bg-orange-600 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-sm transition-all"
            >
              <span>Entrar</span>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
      ...
      <section className="relative bg-[#123498] text-white py-16 sm:py-24 border-b border-slate-200/10">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight leading-[1.1] max-w-4xl font-heading mb-6">
            Impulsamos tu <span className="text-[#FDB907]">crecimiento profesional</span>
          </h1>

          <p className="text-base sm:text-lg md:text-xl text-slate-200/90 font-light max-w-2xl mb-12 leading-relaxed">
            Explora oportunidades laborales y conecta con empresas líderes con el respaldo de Consultora JB.
          </p>
          ...
          >
            <div className="flex-1 px-4 py-2 sm:py-3.5 flex items-center gap-3 border-b md:border-b-0 md:border-r border-slate-100">
              <svg className="w-5 h-5 text-[#123498] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex-1 px-4 py-2 sm:py-3.5 flex items-center gap-3">
              <svg className="w-5 h-5 text-[#F46F0B] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-2 text-xs font-semibold">
            ...
            {TRENDS.map((trend) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full border transition-all ${(trend === activeCategory)
          ...
          <div className="mt-10 max-w-7xl mx-auto px-4">
            <div className="flex flex-wrap items-center justify-center gap-6 md:gap-8">
              {COMPANAS_CONFIANZA.map((emp, index) => (
                <div key={index} title={emp.name} className="flex justify-center shrink-0">
                  {emp.render()}
                </div>
              ))}
            </div>
      ...
      <main id="empleos-recientes" className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">

        <div className="flex items-center justify-between mb-10">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black text-[#123498] tracking-tight font-heading uppercase">
              {activeCategory !== "Todos" ? `Empleos en ${activeCategory}` : "Empleos Recientes"}
            </h2>
            <div className="w-8 h-1 bg-[#F46F0B] mt-2 mb-1 rounded-full" />
            <p className="text-slate-400 text-xs font-bold mt-1">
              Mostrando {displayedJobs.length} de {filteredJobs.length} {filteredJobs.length === 1 ? "vacante disponible" : "vacantes disponibles"}
            </p>
          </div>
        ...
        {displayedJobs.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm max-w-xl mx-auto">
            ...
            <h3 className="font-extrabold text-lg text-[#123498] mb-2 font-heading">No encontramos vacantes</h3>
            <p className="text-[#A3A3A3] text-sm font-medium mb-6">
              Prueba modificando las palabras clave o limpia los campos para ver todas las ofertas disponibles.
            </p>
            <button
              onClick={() => {
                setSearchTerm("");
                ...
                <div>
                  <div className="flex items-start justify-between mb-5">
                    <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100 shrink-0 shadow-inner">
                      <svg className="w-5 h-5 text-[#123498]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                    </div>
                  ...
                  <div className="space-y-2 mb-6">
                    <div className="flex items-center gap-2.5 bg-slate-50 px-3.5 py-2.5 rounded-xl border border-slate-100">
                      <svg className="w-4 h-4 text-[#F46F0B] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      </svg>
                      <span className="text-xs font-bold text-slate-600 line-clamp-1">{job.location}</span>
                    </div>

                    <div className="flex items-center gap-2.5 bg-slate-50 px-3.5 py-2.5 rounded-xl border border-slate-100">
                      <svg className="w-4 h-4 text-[#41C4C0] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2" />
                      </svg>
                      <span className="text-xs font-bold text-[#41C4C0] line-clamp-1">{job.compensation}</span>
                    </div>
                  </div>
                </div>
      ...
      {/* 6. ¿QUIÉNES SOMOS? */}
      <section id="quienes-somos" className="bg-white border-t border-slate-100 py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-[#A3A3A3] text-sm font-semibold max-w-2xl mx-auto leading-relaxed" style={{ fontFamily: "'Lato', sans-serif" }}>
              Somos la <span className="text-[#123498] font-bold">Consultora de Asesoría Empresarial JB</span>, un aliado estratégico para estudiantes y empresas, brindando servicios personalizados de asesoría, capacitación y consultoría.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            ...
            <div className="bg-[#123498] rounded-2xl p-7 text-white hover:shadow-lg transition-all duration-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-black text-lg uppercase tracking-wider font-heading">Misión</h3>
              </div>
              <p className="text-white/85 text-sm leading-relaxed font-sans">
                Ser un aliado estratégico brindando servicios de asesoría, capacitación y consultoría de alta calidad, con honestidad e innovación, para superar desafíos y alcanzar el éxito.
              </p>
            </div>
            ...
            <div className="bg-[#F46F0B] rounded-2xl p-7 text-white hover:shadow-lg transition-all duration-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-black text-lg uppercase tracking-wider font-heading">Visión</h3>
              </div>
              <p className="text-white/85 text-sm leading-relaxed font-sans">
                Ser reconocidos a nivel nacional como referentes en calidad, innovación y honestidad, acompañando a nuestros clientes en su crecimiento y consolidación.
              </p>
            </div>
            ...
            <div>
              <div className="bg-[#2A2A2A] border-l-4 border-[#FDB907] p-5 rounded-r-xl shadow-md">
                ...
                <p className="text-slate-300 text-xs leading-relaxed font-medium">
                  Soluciones empresariales integrales basadas en compromiso, innovación y excelencia. Transformamos negocios con estrategias personalizadas.
                </p>

                {/* Redes Sociales en cajas doradas */}
                <div className="flex items-center gap-2 mt-5">
                  {/* Facebook */}
                  <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg bg-[#a1854a] hover:bg-[#b08f4c] flex items-center justify-center text-white transition-colors">
                    <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                      <path d="M9 8H7v3h2v9h3v-9h3.625L16 8h-3V7c0-.9.2-.9 1-.9H16V3h-2.5C10.5 3 9 4.5 9 7v1z" />
                    </svg>
                  </a>
                  {/* LinkedIn */}
                  <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg bg-[#a1854a] hover:bg-[#b08f4c] flex items-center justify-center text-white transition-colors">
                    <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                      <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.779-1.75-1.75s.784-1.75 1.75-1.75 1.75.779 1.75 1.75-.784 1.75-1.75 1.75zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                    </svg>
                  </a>
                  {/* Instagram */}
                  <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg bg-[#a1854a] hover:bg-[#b08f4c] flex items-center justify-center text-white transition-colors">
                  ...
                  {/* TikTok */}
                  <a href="https://tiktok.com" target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg bg-[#a1854a] hover:bg-[#b08f4c] flex items-center justify-center text-white transition-colors">
                    <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  ...
                  {/* YouTube */}
                  <a href="https://youtube.com" target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg bg-[#a1854a] hover:bg-[#b08f4c] flex items-center justify-center text-white transition-colors">
                    <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <ul className="space-y-3 font-semibold text-xs text-slate-300">
                ...
                <li className="flex items-start gap-2 text-slate-300">
                  <svg className="w-4 h-4 text-[#FDB907] shrink-0 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                ...
                <li className="flex items-center gap-2 text-slate-300">
                  <svg className="w-4 h-4 text-[#FDB907] shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
          ...
          <div className="relative w-full max-w-xl bg-white h-full shadow-2xl flex flex-col justify-between z-10 overflow-hidden rounded-l-2xl border-l border-slate-200">
            ...
            <div className="p-6 sm:p-8 border-b border-slate-100 flex items-start justify-between relative bg-slate-50/50">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center border border-slate-200 shadow-sm shrink-0">
                  <svg className="w-6 h-6 text-[#123498]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
                <div>
            ...
            <div className="flex-1 p-6 sm:p-8 overflow-y-auto space-y-8">
              {applicationSuccess && (
                <div className="bg-[#E6F8F6] border border-[#41C4C0]/30 rounded-xl p-5 text-center text-[#41C4C0]">
                  ...
                  <h4 className="font-extrabold text-sm uppercase tracking-wide">¡Postulación Exitosa!</h4>
                  <p className="text-xs font-semibold text-slate-500 mt-1">
                    Tu postulación ha sido registrada con el respaldo de Consultora JB.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                <ul className="list-none space-y-3">
                  {selectedJob.requirements.map((req, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-slate-600 text-sm font-medium">
                      <div className="w-5 h-5 rounded-full bg-orange-50 border border-orange-100 flex items-center justify-center shrink-0 mt-0.5">
                        <svg className="w-3 h-3 text-[#F46F0B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {selectedJob.benefits.map((ben, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-slate-600 text-sm font-medium">
                      <div className="w-5 h-5 rounded-full bg-teal-50 border border-teal-100 flex items-center justify-center shrink-0 mt-0.5">
                        <svg className="w-3 h-3 text-[#41C4C0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3.5" d="M5 13l4 4L19 7" />
                        </svg>
                      </div>
                      <span>{ben}</span>
            ...
            <div className="p-6 sm:p-8 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <div className="hidden sm:block">
                <span className="text-[10px] text-[#A3A3A3] font-extrabold tracking-wider uppercase block">Publicado</span>
                <span className="text-xs font-bold text-[#123498]">{selectedJob.date}</span>
              </div>

              <button
                onClick={() => handleApplyJob(selectedJob.id)}
                disabled={applicationSuccess}
                className={`w-full sm:w-auto px-10 py-4 font-black uppercase text-xs tracking-wider rounded-full shadow-sm hover:shadow-md flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-[0.98] ${applicationSuccess
